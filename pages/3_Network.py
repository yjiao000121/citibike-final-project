...
st.subheader("Spring-Layout Network Graph")

spring_top_n = st.sidebar.slider(
    "Spring graph: top undirected edges",
    min_value=20,
    max_value=100,
    value=60,
    step=10
...
spring_df = od.copy()
spring_df["station_a"] = spring_df[["start_station_name", "end_station_name"]].min(axis=1)
spring_df["station_b"] = spring_df[["start_station_name", "end_station_name"]].max(axis=1)

edges_undirected = (
    spring_df.groupby(["station_a", "station_b"], as_index=False)
    .agg(total_trips=("trip_count", "sum"))
    .sort_values("total_trips", ascending=False)
)

top_spring_edges = edges_undirected.head(spring_top_n).copy()

G_spring = nx.Graph()
for _, row in top_spring_edges.iterrows():
    G_spring.add_edge(
        row["station_a"],
        row["station_b"],
...
weighted_degree = dict(G_spring.degree(weight="weight"))

# avoid empty graph issue
if len(G_spring.nodes()) == 0:
    st.warning("No edges available for the current spring-layout settings.")
else:
    node_sizes = [max(weighted_degree[node] * 0.15, 80) for node in G_spring.nodes()]

    edge_weights = [G_spring[u][v]["weight"] for u, v in G_spring.edges()]
    ...
    labels = {node: node if node in top_label_nodes else "" for node in G_spring.nodes()}

    pos = nx.spring_layout(G_spring, k=0.8, seed=42)
    ...
    ax.set_title("Spring-Layout Citi Bike Station Network", fontsize=16)
    ax.axis("off")
...
st.write(
    "This abstract network graph shows the structure of the station system without geographic coordinates. "
    "Unlike the map, it emphasizes hub positions, clusters, and how strongly stations connect within the network."
)
